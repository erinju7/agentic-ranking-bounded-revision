"""Score Study 2 listwise runs against James's expert gold (study2_listwise_redesign_spec.md).

Metrics (all vs James gold):
  - graded nDCG@{1,3,6}: relevance grades from the JAMES gold (match=2, borderline=1, no_match=0,
  ...
  - per-call grade kappa: variant's own grades vs James (quadratic-weighted, 3-level) over 36 pairs.
  - Mito top-1 false-positive: 1-case check that rank-1 has James-grade 0 (probes D's preserve).
...
LW = ROOT / "results" / "james_validation" / "listwise" / "listwise_results.json"
GOLD = Path("/Users/macbook/Desktop/gold_standard_JDR.csv")
OUT = ROOT / "results" / "james_validation" / "listwise" / "scores_vs_james.json"
...
def weighted_kappa(pairs):
    # pairs: list of (james_label, variant_grade)
...
def pairwise_kappa(gold):
    ...
    p = ROOT / "results" / "james_validation" / "pairs.json"
    if not p.exists():
        return {}
...
def main():
    ...
    print(f"Study 2 listwise vs James (n_proposals={len(results)}; nDCG n=5, rank n=4)\n")
    ...
    print("\npointwise vs listwise per-call weighted-kappa (vs James):")
    for v in ["A", "B", "D"]:
    ...
    OUT.write_text(json.dumps({"scores": scores, "pairwise_kappa": pw,
                               "notes": "graded nDCG grades from James gold; LLM grades feed kappa only; "
